$(function () {
    ...
    $("#btnChangePass").click(function () {
        var isError = Validate("changepass");
        if (isError == false) {
            $.ajax({
                type: "POST",
                url: urlChangePass,
                data: JSON.stringify({ oldPass: $("#txtOldPass").val(), newPass: $("#txtNewPass").val() }),
...
var InitControl = {
    Main: function () {
        ...
        $("input[data-type=currency]").on("input propertychange paste", function () {
            var value = $(this).val().replace(/[^0-9]/gi, "");
            var _this = $(this);
            setTimeout(function () {
                _this.val(Number(value.replace(/,/g, "")).toLocaleString('en'));
            }, 50)
        });
        ...
        if ($.fn.datetimepicker)
            $(".hasDatepicker").each(function () {
                var attrMinview = $(this).attr("minview") || "datetime";
                var format = $(this).attr("data-date-fomat") || "dd-mm-yyyy hh:ii";
                var minview = attrMinview === "date" ? 2 : attrMinview === "datetime" ? 0 : 0
                var _this = $(this);
                $(this).datetimepicker({
                    format: format,
                    autoclose: true,
                    ...
                    onChange: function (date) {
                        alert(date);
                        angular.element(_this).triggerHandler('input');
                    }
                });
            })
    ...
    Checkbox: function () {
        $("input[data-target]").on("ifChanged", function () {
            $($(this).attr("data-target")).val($(this).is(":checked"));
        });
    },
    ...
    FormatGrid: function () {
        $.extend(jsGrid.Grid.prototype, {
            ...
            pagePrevText: "<span class='ti-arrow-left'></span>",
            pageNextText: "<span class='ti-arrow-right'></span>",
            pageNavigatorNextText: "&#8230;",
            pageNavigatorPrevText: "&#8230;",

            pagerContainerClass: "mt-3",
            pagerClass: "pagination justify-content-center",
            pagerNavButtonClass: "page-item",
            pagerNavButtonInactiveClass: "disabled",
            pageClass: "page-item",
    ...
    UploadFile: function (element, success, progress) {
        element.fileupload({
            dataType: "json",
            add: function (e, data) {
            ...
            progress: function (e, data) {
                if (progress != undefined) progress(e, data);
            },
...
function Validate(idDiv) {
    ...
    $((idDiv !== undefined ? "#" + idDiv + " " : "") + "[data-required=true]").each(function () {
        var error = $(this).attr("data-key") || "null";
        var value = $.trim($(this).val());
        // check input null
        if (value === "") {
            addError($(this), resources[error].replace("{field_name}", $(this).attr("placeholder") || ""));
        } else {
            // Check độ dài tối thiểu của gtri
            if ($(this).attr("data-length") !== undefined && value.length < parseInt($(this).attr("data-length"))) {
                addError($(this), resources.text_length.replace("{field_name}", $(this).attr("placeholder") || "").replace("{length}", $(this).attr("data-length")));
            }
            ...
            if ($(this).attr("data-type") !== undefined) {
                ...
                if (type === "password") {
                    if (value.length < 1)
                        addError($(this), resources.password_length);
                    else {
                        // Check confirm password
                        if ($(this).attr("data-compare") !== undefined) {
                            if ($("#" + $(this).attr("data-compare")).val() !== "" && value !== $("#" + $(this).attr("data-compare")).val()) {
...
function addError(_this, error) {
    if (_this.prop("tagName") === "SELECT") {
        ...
        if (_this.next("div.help-block").length < 1) {
            _this.after("<div class='help-block' style='color:red'></div>");
        }
        _this.addClass("input-validation-error");
        _this.next().html('<ul role="alert"><li>' + error + "</li></ul>").show();
